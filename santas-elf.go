...
func handleDay(token string, day, year int, placeholder bool) error {
	...
	if err = createScript(path, fmt.Sprintf(goScriptInitialContent, day), fmt.Sprintf("day%d.go", day), false); err != nil {
		return err
	}

	if err = createScript(path, fmt.Sprintf(jsScriptInitialContent, day), fmt.Sprintf("day%d.js", day), false); err != nil {
		return err
	}
